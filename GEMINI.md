...
## Overview
This file contains Gemini-specific guidance for working on the Modern Shell Setup project.
...
### Local Invocation
The `g` alias invokes the Gemini CLI in this shell configuration:
```zsh
alias g='gemini'
```

### API Key Location
If using the Gemini CLI, API keys should be set in `~/.zshrc.local` (not committed to git):
```zsh
